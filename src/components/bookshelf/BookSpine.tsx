 ...
 */
function BookSpine({ book, isOpen, isHidden, onOpen }: BookSpineProps) {
  ...
  const style: CSSProperties = {
    ...
    ...(book.spineImage
      ? { ['--spine-image' as string]: `url("${book.spineImage}")` }
      : {}),
  }

  return (
    <div
      className={`book${isOpen ? ' is-empty' : ''}${isHidden ? ' is-hidden' : ''}`}
      style={style}
    ...
    >
      <button
        ...
        aria-label={`${book.title} by ${book.author}. Open details.`}
        aria-haspopup="dialog"
        aria-expanded={isOpen}
        onClick={() => buttonRef.current && onOpen(book, buttonRef.current)}
      >
        <span
          className={`book__spine${book.spineImage ? ' book__spine--image' : ''}`}
        >
