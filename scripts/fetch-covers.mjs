...
const termFor = (book) => QUERIES[book.id] || `${book.title} ${book.author.split(',')[0]}`
...
async function main() {
    ...
    for (const book of books) {
        ...
        if (buf) {
            const webp = await sharp(buf).webp({ quality: 85, effort: 6 }).toBuffer()
            await writeFile(join(COVERS_DIR, `${book.id}.webp`), webp)
            book.coverUrl = `/covers/${book.id}.webp`
