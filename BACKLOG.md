...
| U2  | ~~P0~~ **Done**  | M     | Hero animation refinement   | `useHeroAnimations` reworked; `prefers-reduced-motion` honoured via `motionPresets()` in `src/lib/motion.ts` (`HomeClient.tsx` is the reference call site).                     |
| U3  | ~~P1~~ **Done**  | S     | Page transitions            | `src/app/[lang]/template.tsx` — a Server Component wrapping children in `.page-enter`; the keyframes and the reduced-motion opt-out live in `globals.css`.                      |
...
| I3  | ~~P1~~ **Done** | S     | Custom 404 page                    | `src/app/global-not-found.tsx` (needs `experimental.globalNotFound`); resolves the locale from the `x-locale` header the middleware sets.                                                                                                                                                   |
...
| S6  | **P1** (partial) | S     | OG image per route          | Root and `[lang]/services` have an `opengraph-image.tsx`. Project pages set `openGraph.images` to the real project screenshot in `generateMetadata`, which takes precedence over the file convention — a generated card for them was written, never served, and has been removed. Blog posts still fall back to the root image. |
